...
void Manager::fleeingFromAmbush(){
    ...
    //Set the data
    menu->setData("Coward", about, options, &cowardMenu);
    if(menu->inMenu){
...
void Manager::fightingFromAmbush(int numBandits, int banditDamage){
	...
	if(Manager::getRandomAliveMercIndex() < 0){
	    ...
	    //Set the data
	    menu->setData("Close but no pipe", about, options, &diedMenu);
	}else{
		gold += goldGain;
		food += foodGain;
		beer += beerGain;
	    ...
	    options.push_back(resumeOpt);
	    //Set the data
	    menu->setData("Great slayin' ", about, options, &fightWonMenu);
	}
...
void Manager::fightingFromMerchant(int numGaurds, int gaurdDamage){
	...
	if(Manager::getRandomAliveMercIndex() < 0){
	    ...
	    //Set the data
	    //TODO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	    menu->setData("Skilled merchant", about, options, &diedMenu);
	}else{
		gold += goldGain;
		food += foodGain;
		beer += beerGain;
	    ...
	    options.push_back(resumeOpt);
	    //Set the data
	    menu->setData("Rad robbery", about, options, &merchantRobbedMenu);
	}
...
void Manager::buyMercenaryTrail(int cost)
{
	if(rand() % 10 == 1){
	    ...
	    options.push_back(resumeOpt);
	    //Set the data
	    menu->setData("Oops", about, options, &oopsRanMenu);
	}else{
		if(gold >= cost){
